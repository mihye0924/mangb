...
<script setup>
import { data as next } from './next.data.js'
import Avatar from '../../.vitepress/theme/components/Avatar.vue'
</script>

<section v-if="next.length">
  <h1 class="mt-12 mb-8 !text-3xl font-bold">{{ next[0].url.split('/')[2].toUpperCase() }} 에 대한 정보</h1>
  <ul class="grid grid-cols-1 gap-4 !px-0 !list-none sm:grid-cols-2 lg:grid-cols-3"> 
    <li v-for="post in next" class="mb-6 border"> 
