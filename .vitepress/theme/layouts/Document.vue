      ...
      <div class="content">
        <div class="content-container">
          <slot name="doc-before">
            <ul v-if="frontmatter.categories">
              <li v-for="category in frontmatter.categories" class="inline-block mr-1 mb-1.5 last:mb-5">
                <a :href="`/categories/${category}`" class="block px-2 rounded-full text-sm leading-6 bg-zinc-200 dark:bg-zinc-700">{{ category }}</a>
              </li>
            </ul>
            <h1 class="mb-5 text-4xl sm:text-5xl leading-tight font-bold">{{ page.title }}</h1>
            <Avatar :name="frontmatter.author" :date="frontmatter.created || frontmatter.date" class="mb-20" />
          </slot>
          <main class="main">
            <Content
              class="vp-doc"
            ...
            />
            <div v-if="frontmatter.members" class="members px-5 py-4 mt-16 border border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800 rounded-lg">
              <template v-if="frontmatter.presenter">
                <div class="flex items-center gap-x-2 mb-4">
                  <p class="text-sm font-semibold">발표자</p>
                </div>
                <div class="flex mb-10">
                  <Avatar :name="frontmatter.presenter" />
                </div>
              </template>
              <div class="flex items-center gap-x-2 mb-4">
                <p class="text-sm font-semibold">참석자</p>
                <Badge type="info" :text="frontmatter.members.length || 0" />
              </div>
              <div class="flex">
                <Avatar v-for="(member, idx) in frontmatter.members.reverse()" :name="member" type="simple" class="avatar relative" :style="`z-index: ${frontmatter.members.length - idx}`" />
              </div>
            </div>
          </main>
            ...
            <nav v-if="prev || next" class="sm:flex py-8 border-t border-zinc-200 dark:border-zinc-700">
              <div v-if="prev" :class="next ? 'sm:w-1/2' : 'w-full'">
                <a class="flex" :href="prev.url">
                  <div class="flex flex-col py-1.5 sm:mr-3">
                    <span class="text-sm">Previous page</span>
                    <span class="mt-2 text-xl font-bold leading-6">{{ prev.title }}</span>
                  </div>
                </a>
              </div>
              <div v-if="next" class="mt-6 sm:mt-0" :class="prev ? 'sm:w-1/2' : 'w-full'">
                <a class="flex sm:flex-row-reverse" :href="next.url">
                  <div class="flex flex-col py-1.5 sm:ml-3 sm:items-end sm:text-right">
...
.members :deep(.avatar:first-child) {
  margin-left: -5px;
}
.members :deep(.avatar:before) {
  content: '';
  position: absolute;
  inset: 0;
...
.dark .members :deep(.avatar:before) {
